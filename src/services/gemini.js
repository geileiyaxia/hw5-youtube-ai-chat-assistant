...
export const chatWithJsonTools = async (history, newMessage, jsonFields, executeFn, imageParts = []) => {
  const systemInstruction = await loadSystemPrompt();
  const model = genAI.getGenerativeModel({
    model: MODEL,
  ...
  for (let round = 0; round < 5; round++) {
    ...
    if (toolResult?._generateImage) {
      generateImageResult = toolResult;
    }
  ...
  return { text: response.text(), charts, videoCards, toolCalls, generateImageResult };
};
...
export const generateImageWithGemini = async (prompt, anchorImageBase64 = null, mimeType = 'image/png') => {
  ...
  if (anchorImageBase64) {
    parts.push({
      inlineData: { mimeType, data: anchorImageBase64 },
    });
  }
  ...
  return { imageData, mimeType: imageMimeType, text: textContent };
};
